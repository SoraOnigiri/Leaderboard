...
class User(commands.Cog):
    ...
    def get_coingecko_tokens(tokens)
    ...
    def get_token_price(id):
        ...
        r = requests.get(f"https://api.coingecko.com/api/v3/coins/{id}/market_chart/range?vs_currency={currency}&from={str(pfrom)}&to={str(pto)}")
        results = r.json()
    ...
    async def get_trade_id(userid):
        # 1. Retrieve last trade id from user
        return 0

    async def add_trade(userid, tradeid, openDate, openPrice, ticker, openReason):
        deleted = False
        isOpen = True 

    async def update_user(userid, tradeid): 
        pass

    async def get_open_trades(userid):
        pass

    async def get_closed_trades(userid):
        pass

    async def get_total_profit(userid):
        pass

    async def delete_id(userid, tradeid):
        pass
